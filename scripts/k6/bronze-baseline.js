/**
 * Bronze baseline load test — 50 concurrent virtual users (VUs).
 *
 ...
 *
 * Prerequisites: API running (e.g. `uv run run.py`), PostgreSQL up.
 * Run `uv run load_seed.py` once so seed short codes and rows exist.
 *
 * Run:
 *   k6 run scripts/k6/bronze-baseline.js
 *
 * Optional:
 *   BASE_URL=http://127.0.0.1:5000 k6 run scripts/k6/bronze-baseline.js
 *   k6 run --summary-export=scripts/k6/summary.json scripts/k6/bronze-baseline.js
 */
...
export const options = {
  // 50 concurrent users for the whole test window
  vus: 50,
  duration: "1m",
};
